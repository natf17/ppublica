...
console.log("&&&&&&&&&&&&&&&&&");
			...
			for(var i = 0; i < dayArray.length; i++) {
				...
				newTimeSlotsArray = [];
				// in each timeSlot...
					for(var j = 0; timeSlotsArray.length; j++) {
						timeSlotV = timeSlotsArray[j];
							if(typeof timeSlotV != 'undefined' && timeSlotV["publishers"].length >= 1) {
								
								newTimeSlotsArray.push(timeSlotV);

							}
					
					}
					
					dayArray[i]["timeSlots"] = newTimeSlotsArray;
...
console.log("&&&&&&&&&&&&&&&&&");
			...
			for(var i = 0; i < dayArray.length; i++) {
				...
				for(var j = timeSlotsArray.length - 1; j >= 0; j--) {
					//timeSlotV = timeSlotsArray[j];
					if(timeSlotsArray[j]["publishers"].length < 1) {
						// remove
						console.log("removed");
						//console.log(dayArray[i]["timeSlots"]);
		...
		(function submitButton() {
			...
			submitDiv.addEventListener("click", function(event) {
				...
				reqPost.open("PUT", "/PPublica/api/week/" + weekId, true);
		  		...
		  		reqPost.addEventListener("load", function() {
		  			...
		  			if(reqPost.status >= 300 || reqPost.status < 200) {
		  		    	...
		  		    	document.body.appendChild(document.createTextNode(response));
		  	  			...
		  	  			window.location.href="/PPublica/schedules/" + weekId;
		  	  		}
